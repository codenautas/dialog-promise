...
function confirmPromise(message, opts){
    opts = opts||{};
    receiveOpts(opts, DialogPromise.defaultOpts);
    var elementsList=[message].concat(opts.buttonsDef||confirmPromise.defaults.buttons);
...
function miniMenuPromise(menu, opts){
    return dialogPromise(function(mainElement,done){
        ...
        menu.forEach(function(menuOption){
            ...
            (menuOption.displayFun||miniMenuPromise.defaultFun)(row, menuOption, done, opts);
        });
...
miniMenuPromise.defaultFun = function defaultFun(row, menuOption, done, opts){
    ...
    var doDone=function(){
        if(menuOption.doneFun){
            var mayBeAPromise = menuOption.doneFun(row, menuOption, done, opts);
            if(mayBeAPromise instanceof Promise){
                mayBeAPromise.then(done, done);
            }
        }else{
